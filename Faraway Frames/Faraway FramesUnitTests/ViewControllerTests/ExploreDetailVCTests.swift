...
struct ExploreDetailVCTests {
    ...
    func exploreDetailVC_viewDidAppear_syncsValuesWithThoseInDatabase() throws {
        let (sut, context) = try makeSUTAndContext()
        ...
        sut.filmDetailViewModel.handleFilmUpdate(filmMO)
        ...
        if case .content(let displayModel,_) = sut.filmDetailViewModel.currentState {
            ...
            Issue.record("Expected .content(let displayModel,_) but got \(sut.filmDetailViewModel.currentState)")
        }
    }
    ...
    func exploreDetailVC_tapRetryButtonOnErrorConfig_callsVMUpdateStatusAgain(
        ...
        #expect(sut.filmDetailViewModel.attemptingToUpdateFilm == true, "Should be true because `updateStatus` was called from the retry button.")
    }
    ...
    func exploreDetailVC_tapCancelButtonOnAlert_reloadsFilmContentAndUpdatesState(
        ...
        #expect(sut.filmDetailViewModel.currentState == .content(displayModel: displayModel, image: nil), "Should have returned to content state (note: image is nil because mockImageLoader didn't load image in test setup).")
    ...
    private func makeSUTAndContext() throws -> (
        sut: ExploreDetailVC,

...
struct DecodingAllFilmsTests {
    ...
    @Test func decodingJSON_fromBundleFile_toFilmsArray_successfullyReturnsSecondFilmWithCorrectOriginalTitle() throws {
        let films = try JSONHelper.loadAndDecodeFilmsFromJSON()
        #expect(films[1].originalTitle == "火垂るの墓", "Should return the original title of the second film.")
    }
    
    @Test func decodingJSON_fromBundleFile_toFilmsArray_successfullyReturnsThirdFilmWithCorrectOriginalTitleRomanised() throws {
        let films = try JSONHelper.loadAndDecodeFilmsFromJSON()
        #expect(films[2].originalTitleRomanised == "Tonari no Totoro", "Should return the original title romanised of the third film.")
    }
    
    @Test func decodingJSON_fromBundleFile_toFilmsArray_successfullyReturnsFourthFilmWithCorrectMovieBanner() throws {
        let films = try JSONHelper.loadAndDecodeFilmsFromJSON()
    ...
    @Test func decodingJSON_fromBundleFile_toFilmsArray_successfullyReturnsFifthFilmWithCorrectReleaseDate() throws {
        let films = try JSONHelper.loadAndDecodeFilmsFromJSON()
        #expect(films[4].releaseDate == "1991", "Should return the release date of the fifth film.")
    }
    
    @Test func decodingJSON_fromBundleFile_toFilmsArray_successfullyReturnsSixthFilmWithCorrectRunningTime() throws {
        let films = try JSONHelper.loadAndDecodeFilmsFromJSON()
